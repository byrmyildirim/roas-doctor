...
export default function Shell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen bg-white text-zinc-900 font-sans selection:bg-blue-500/30 transition-colors duration-300">
      {/* Sidebar */}
      <aside className="w-64 border-r border-zinc-200 flex flex-col bg-zinc-50/50 transition-colors">
        ...
        <nav className="flex-1 px-4 py-4 space-y-1">
          ...
          {navItems.map((item) => (
            <Link
              key={item.name}
              href={item.href}
              className="group flex items-center justify-between px-3 py-2 text-sm font-medium rounded-xl transition-all duration-200 hover:bg-zinc-200/50 text-zinc-600 hover:text-black"
            >
              <div className="flex items-center gap-3">
                <item.icon className="w-4 h-4 transition-colors group-hover:text-blue-600" />
                {item.name}
              </div>
        ...
        <div className="p-4 border-t border-zinc-200">
          <button className="flex items-center gap-3 px-3 py-2 w-full text-sm font-medium text-zinc-600 hover:text-black hover:bg-zinc-200/50 rounded-xl transition-all duration-200">
